...
contract("Tweeter", (accounts) => {
	...
	beforeEach(async () => {
		...
		await Promise.all(
			user1Tweets.map((tweet) => tweeter.tweet(tweet, { from: user1 })),
			user2Tweets.map((tweet) => tweeter.tweet(tweet, { from: user2 })),
		)
	})
	...
	it("should send message", async () => {
		const receipt = await tweeter.sendMessage("Message 1", user2, {
			from: user1,
		})
	...
	it("should send a message from approved operator", async () => {
		await tweeter.allow(user2, { from: user1 })
		const receipt = await tweeter.sendMessageFrom("Message", user1, user2, {
			from: user2,
		})
		expectEvent(receipt, "MessageSent", {
			id: web3.utils.toBN(0),
